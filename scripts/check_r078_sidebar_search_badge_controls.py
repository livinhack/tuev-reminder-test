#!/usr/bin/env python3
"""Validate r078 sidebar search/badge control cleanup."""
...
if (ROOT / "REMINDER_VERSION.txt").read_text(encoding="utf-8").strip() != "r078":
    fail("REMINDER_VERSION must be r078")
...
for needle in required:
    if needle not in panel:
        fail(f"missing r078 sidebar control marker: {needle}")
...
if 'grid-template-columns: minmax(240px, 1fr);' not in panel:
    fail("toolbar must be a single search-column grid in r078")

print("r078 sidebar search/badge controls check passed")
