#!/usr/bin/env python3
"""Validate r036 Sidebar modal form and input focus fix."""
...
def fail(message: str) -> None:
    raise SystemExit(f"r036 modal form focus fix check failed: {message}")


def main() -> None:
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    if manifest.get("version") != "0.1.0-r036":
        fail("manifest version must be 0.1.0-r036")
    if VERSION.read_text(encoding="utf-8").strip() != "r036":
        fail("REMINDER_VERSION.txt must be r036")
    ...
    print("r036 modal form focus fix check OK")
