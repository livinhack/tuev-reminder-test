#!/usr/bin/env python3
"""Validate r049 Sidebar row actions and sortable table headers."""
...
def fail(message: str) -> None:
    raise SystemExit(f"r049 sidebar row action/sort header check failed: {message}")
...
def main() -> None:
    ...
    if "Nur die Drei-Punkte-Schaltfläche" not in docs:
        fail("r049 docs must describe row click removal")
    if "Spaltenüberschriften" not in docs:
        fail("r049 docs must describe sortable headers")

    print("r049 sidebar row action/sort header check OK")
