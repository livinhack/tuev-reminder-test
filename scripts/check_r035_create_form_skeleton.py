...
def main() -> int:
    ...
    assert VERSION.read_text(encoding="utf-8").strip() == "r051"
    assert manifest["version"] == "0.1.0-r051"
    ...
    for needle in forbidden:
        if needle in js:
            raise AssertionError(f"r051 Sidebar form skeleton must not include {needle!r}")
