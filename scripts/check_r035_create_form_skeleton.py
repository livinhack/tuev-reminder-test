...
def main() -> int:
    ...
    assert VERSION.read_text(encoding="utf-8").strip() == "r045"
    assert manifest["version"] == "0.1.0-r045"
    ...
    for needle in forbidden:
        if needle in js:
            raise AssertionError(f"r045 Sidebar form skeleton must not include {needle!r}")
