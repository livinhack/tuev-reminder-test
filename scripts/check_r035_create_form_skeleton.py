...
def main() -> int:
    ...
    assert VERSION.read_text(encoding="utf-8").strip() == "r042"
    assert manifest["version"] == "0.1.0-r042"
    ...
    for needle in forbidden:
        if needle in js:
            raise AssertionError(f"r042 Sidebar form skeleton must not include {needle!r}")
